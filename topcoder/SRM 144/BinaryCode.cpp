...
class BinaryCode {
private:
	char elem(const string& str, int index) {
		if (index >= 0 && index < str.length()) return str[index];
		return '0';
	}
	void check(string& a, const string& message) {
		int len = message.length();
		for (int i = 0; i < len - 1; i++) {
			char c = (message[i]) - (elem(a, i)-'0') - (elem(a, i - 1) - '0');
			if (c - '0' >= 2) {
				a = "NONE";
				return;
			}
		...
		if (message[len - 1] != a[len - 1] + (a[len - 2] - '0')) a = "NONE";
	}
public:
